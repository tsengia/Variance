...
class Measure():
    def __init__(self, value, unit, accuracy=6):
        if not isinstance(unit, UnitModel):
            raise TypeError(
                "unit passed to Measure must be of type UnitModel!")
        self.unit = unit
        if isinstance(
                value,
                Measure):  # TODO: Check that units are of the same dimensions
            if not value.unit.dimension == self.unit.dimension:
                raise TypeError(
                    "Measurement passed is not of the same unit dimension!")
            self.value = float(
                round(
                    value.value *
            ...
            raise TypeError(
                "value passed to Measure must be int, float, or Measure type!")
    ...
    def __eq__(self, rhs):
        if isinstance(rhs, Measure):
            # Check to make sure they measure the same thing (both are length,
            # or mass or volume, etc.)
...
class MeasurementParser():
    _measure_split_regex = re.compile(
        "(?<=[0-9])[\\s](?=([a-zA-Z.]+(\\s[a-zA-Z]+[.]*)?))")
    _measure_space_regex = re.compile("([0-9])([a-zA-Z])")

    @staticmethod
    def parse(text):
        # Put a space between the value and units if there wasn't one already
        spaced_text = re.sub(
            MeasurementParser._measure_space_regex,
            r"\1 \2",
            text)
        # split at the space between the value and the unit
        s = MeasurementParser._measure_split_regex.split(spaced_text)
        if len(s) < 2:
